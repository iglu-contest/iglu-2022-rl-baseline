...
class GridWorld(Env):
    ...
    def initialize_world(self, starting_grid, initial_poisition):
       # raise Exception(list(zip(*starting_grid)))
    ...
    def reset(self):
        ...
        position += list(self.initial_rotation)
        print(position)
        obs = {
            'agentPos': np.array(position, dtype=np.float32),
...
class Visual(Wrapper):
    def __init__(self, env: Env) -> None:
        ...
        self.size = 256
        self.glob_step = 0
        self.observation_space['obs'] = Box(low=0, high=1, shape=(self.size, self.size, 3), dtype=np.float32)
